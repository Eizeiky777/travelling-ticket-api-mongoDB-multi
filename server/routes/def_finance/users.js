...
router.post("/register", (req, res) => {
    ...
    User.findOne({ email: email, platform: "def_finance" })
        .then((savedUser) => {
            if (savedUser && savedUser.platform === 'def_finance') {
...
router.post("/login", (req, res) => {
    User.findOne({ email: req.body.email, platform: "def_finance" }, (err, user) => {
        if (!user)
            return res.json({
                loginSuccess: false,
...
router.get("/logout", auth, (req, res) => {
    User.findOneAndUpdate({ _id: req.user._id, platform: "def_finance" }, { token: "" }, (err, doc) => {
        if (err) return res.json({ success: false, err });
...
router.get("/get_all_users/:order/:sortBy/:limit/:skip", authAdmin, (req, res) => {
    ...
    User.find({ platform: "def_finance" })
        .sort([[sortBy, order]])
        .skip(skip)
        .limit(limit)
        .exec((err, users) => {
