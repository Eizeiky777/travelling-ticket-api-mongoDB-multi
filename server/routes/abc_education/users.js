...
router.post("/register", (req, res) => {
    ...
    User.findOne({ email: email })
        .then((savedUser) => {
            if (savedUser && savedUser.platform === 'abc_education') {
...
router.post("/login", (req, res) => {
    User.findOne({ email: req.body.email, platform: "abc_education" }, (err, user) => {
        if (!user)
            return res.json({
                loginSuccess: false,
...
router.get("/logout", auth, (req, res) => {
    User.findOneAndUpdate({ _id: req.user._id, platform: "abc_education" }, { token: "" }, (err, doc) => {
        if (err) return res.json({ success: false, err });
...
router.get("/get_all_users/:order/:sortBy/:limit/:skip", authAdmin, (req, res) => {
    ...
    User.find({ platform: "abc_education" })
        .sort([[sortBy, order]])
        .skip(skip)
        .limit(limit)
        .exec((err, users) => {
